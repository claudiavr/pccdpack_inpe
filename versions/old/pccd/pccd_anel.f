...
c       Numero de estrelas=1000    *    06 Jun 95    *    VEM - CVR
c
c	Acrescentando opcao de fazer polarimetria de aberturas
c	em forma de anel - CVR 30/04/2000
c
c       ano(# estrelas, pos. lamina, aberturas)
c
	implicit real*8 (a-h, o-z)
	dimension ano(1000,64,18), ane(1000,64,18), skyo(1000,64), skye(1000,64)
	dimension ap(18), areao(1000,64,18), areae(1000,64,18), a(60)
	dimension sko(64), ske(64), ao(64), ae(64), areo(64), aree(64)
	dimension z(64),areaso(1000,64),arease(1000,64)
        integer nimages,anular
...
c
c ** opcao de abertura anular - inicio
c
				if (anular.eq.1) then
				 if (k.ne.1) then
				  ao(i)=ano(j,i,k)-ano(j,i,k-1)
				  if (nimages.eq.2) ae(i)=ane(j,i,k)-ane(j,i,k-1)
				  areo(i)=areao(j,i,k)-areao(j,i,k-1)
				  if (nimages.eq.2) aree(i)=areae(j,i,k)-areae(j,i,k-1)
				 end if
				end if
c ** opcao de abertura anular - final
c
			end do
			npsky=npsky/2.
