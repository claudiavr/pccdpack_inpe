...
begin
	...
	string arquivo
	string temp1, temp2
	string lixo1, lixo2, lixo20, lixo21
	string imagem
	...
	real pmedio
	int i1, npix, inut, lixo22, lixo23
	bool ver, ver1
	string rot1, rot2
	int numer, interv
...
# eh subtraido de 180
	...
	IF (model == "PA" && threemax >= 180.) {
# SE ThreeMAX eh >= 180, entao todo ponto do angulo com valor abaixo de threemin
# eh sumado de 180 (por AAPQ)
		imcalc (input=arq2, output="tmp$temp111.fits",
			   equals="if im1 .le. "//min3//" then im1+180. else im1",
			   verbose=no)
		imdelete(arq2, ver-)
		imrename("tmp$temp111.fits", arq2)
	}
...
## incluindo calculo do average com theta corregido
unlearn average
print("")
print("Running AVERAGE ...")
print("lbdmin: ", lbdmin)
print("lbdmax: ", lbdmax)
lixo20 = mktemp("tmp$mosaic")
copy(arqan,lixo20)
delete(arqan)
copy(arq2,arqan)
average(tip,interv,numer,
        lbdmin=lbdmin,
	lbdmax=lbdmax,
	corflat=corflat,
	coreq=coreq,
