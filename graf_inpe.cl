...
#  Agosto de 2011 - CVR
#
# O graf foi modificado para "descobrir" se o log de entrada foi
# criado com a opcao de novo modulo. Nesse caso, em que lamina
# retardora gira em sentido contrario ao correto, os graficos sao
# feitos de modo a levar isso em conta. Isto eh,
# posicao 1 da lamina = -22.5 = 360 - 22.5 = 337.5
# posicao 2 da lamina = -45.0 = 315
# e assim por diante.
#
#
#################################################

procedure graf_inpe (filein,starin,aperturein)
...
bool   metafile = no {prompt="create output EPS file?"}
...
begin
     ...
     string kq,ku,ks,kp,kt,kst,waveplate,kv,ksv,fileps,present_dir,file5
...
#             stop
	 ...
	 m[13] = m13 ; m[14] = m14 ; m[15] = m15 ; m[16] = m16
         ...
         # le se foi usado novo modulo polarimetrico (.log)
         if (nl == 22) {
             line22 = fscan (line, lix, lix, lix, new_module)
         ...
         if (nl == 26) {
             ...
             if (eaperture >= apertures+apermin || eaperture < apermin) {
                 print(line)
                 print(apermin,apertures,eaperture)
...
#     bot1 = -top1
     ...
     for (i = 1; i <= 16; i+= 1) 
          if (zz[i] != 0) {
	      if (new_module == 1) 
                print ((360.d0-step*(i-1)),zz[i],s, >> file2)
	      else print (step*(i-1),zz[i],s, >> file2)
              if (zz[i] > top2)
                  top2 = zz[i] 
              if (zz[i] < bot2)
                  bot2 = zz[i]   
...
#     
     print ("erase", >> file3)
     if ((waveplate == "quarter") || (waveplate == "other") || (waveplate == "v0other"))
          print ("location .12 0.95 .10 .70", >> file3)
     else print ("location .12 0.95 .10 .80", >> file3)
	 ...
	 print ("expand 1; label "//present_dir//"/"//imagem//" star "//estar, >> file3
